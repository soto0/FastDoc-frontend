...
type RepoSelection = Pick<IRepo, 'repo' | 'owner'>;

const SearchPanel = () => {
    const { params, updateParams } = useSearchParams();
    const [open, setOpen] = useState(false);
    const [repo, setRepo] = useState<RepoSelection>();
    ...
    useEffect(() => {
        ...
        if (repoParam && ownerParam) {
            setRepo({ repo: repoParam, owner: ownerParam });
        } else {
            setRepo(undefined);
        }
    ...
    return (
        <>
            <section className='search-section'>
                <form className='search-form animate-slide-down' role='search' onSubmit={handleSubmit}>
                    <FieldGroup className='flex flex-row items-end'>
                        <Field onClick={() => setOpen(true)}>
                            <FieldLabel>Название пакета</FieldLabel>
                            <Input placeholder='next.js, vue 3.5...' readOnly value={repo?.repo ?? ''} className='py-5' />
                        </Field>
                        <TagSelector repo={repo?.repo} owner={repo?.owner} tag={tag} setTag={setTag} />
                        <Field className='w-fit'>
                            <Button type='submit' className='px-10 py-5 gap-2' size='icon-lg' disabled={repo == null || tag == null || isLoading}>
                                <Search aria-hidden />
                                Поиск
                            </Button>
                        </Field>
